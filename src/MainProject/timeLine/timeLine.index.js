...
const TimeLine = () => {
    ...
    const accept = (hichhikeId) => {
        post(APIPath.hichhike.passengerPending + `?token=${authToken.get()}`, {
            id: hichhikeId,
            accept: 1
        }).then((data) => {
            if (responseValidator(data.status) && data.data) {
                            ...
                            suggestion.map((item) => (
                                <div className="time-line-card">
                                    ...
                                    <div className='content'>
                                        ...
                                        <p className="follow-traveler">تعداد
                                            مسافر: {item.fellow_traveler_num && item.fellow_traveler_num.length > 20 ? item.fellow_traveler_num.substring(0, 20) + '...' : item.fellow_traveler_num}</p>
                                        <p className="time">زمان
                                            سفر: {item.jcreated && item.jcreated > 20 ? item.jcreated.substring(0, 20) + '...' : item.jcreated}</p>
                                        <Tooltip placement="right">
                                        ...
                                        <div className="end-line-button">
                                            <Button className="join" onClick={() => join(item.id)}>اضافه شدن</Button>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <List.Item>
                                    ...
                                    {!item.accepted ?
                                        <div className="end-line">
                                            {/*<Button className="add-button-deny" onClick={() => deny(item.id)}>*/}
                                            {/*    رد کردن*/}
                                            {/*</Button>*/}
                                            <i className="material-icons add-button-deny" onClick={() => deny(item.id)}>clear</i>
                                            <i className="material-icons add-button" onClick={() => accept(item.id)}>person_add</i>
                                            {/*<Button className="add-button" onClick={() => accept(item.id)}>*/}
                                            {/*    اضافه شدن*/}
                                            {/*</Button>*/}
                                        </div> :
